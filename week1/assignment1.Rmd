...
Visualize the number of publications discussing sea level rise and the dates of each publication. The API searches based on both headlines and snippets so the dates are the same for both headlines and snippets. 
...
```
The dates of publication are the same 
...
headlines_graph <- headlines_tokenized |> 
  ...
  ggplot(aes(n, clean)) +
  geom_col() +
  labs(title = "Word frequencies in NYT article headlines about Sea Level Rise",
       y = NULL,
       x = "Count") + 
  theme_minimal()
...
headlines_graph
```
Compare the graphs from showing the most words that show up in the snippets versus the headlines.
...
```
There are many more words with a count larger than 5 when analyzing snippets compared to the number of words with a count larger than 5 when analyzing headlines. This makes sense since snippets are more text than headlines. The word climate, change, and trump are found more than 5 times in both the snippets and the headlines. The remaining words still revolve around the topic of sea level rise but are not included as found more than 5 times in both instances. 
